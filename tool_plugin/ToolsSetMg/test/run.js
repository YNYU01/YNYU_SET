...
if (!IS_PLUGIN_ENV) {
  ...
  window.addEventListener('load', () => {
    // 初始化tabPick
    const savedTabPick = storageMix.get('tabPick_mg');
    if (savedTabPick && typeof viewPage === 'function') {
      viewPage(savedTabPick);
    }
    
    // 初始化userSkillStar
    const savedUserSkillStar = storageMix.get('userSkillStar_mg');
  ...
  // 插件环境：异步获取
  toolMessage(['tabPick_mg','getlocal'],PLUGINAPP);
  toolMessage(['userSkillStar_mg','getlocal'],PLUGINAPP);
}
...
window.addEventListener('message',(message)=>{
  ...
  if(ISWORK_TIME){
    switch (type){
      case 'tabPick_mg': viewPage(info);break
      case 'userSkillStar_mg':userSkillStar = info || []; moveSkillStar(userSkillStar);break
