    ...
    this.doc = {
      toolsset:{
        create:{
          ...
          error:[
            ["上传文件格式不支持: 请检查文件格式是否符合要求, 图片需为常见格式, 表格需为 /++ .csv | .xlsx ++/",
            "Unsupported file format: Check if the file format meets requirements, images must be common formats, tables must be .csv | .xlsx"],
        ...
        sheet:{
          ...
          help:[
            ["p",
              ...
              ["br","",""],
              ["p",
              "为弥补组件属性的局限性问题, 可使用【标签属性映射】来完成 /++ #xxx.fill(填充色值) | #xxx.stroke(描边色值) | #xxx.fillStyle(填充样式名) | #xxx.strokeStyle(描边样式名)| #xxx.visible(可见性true/false) | #xxx.opacity(透明度0~1) | #xxx.fontSize(字号) | #xxx.instance(切换变体) ++/ 的参数化控制",
              "Use Tag Property Mapping to overcome component property limitations: /++ #xxx.fill | #xxx.stroke | #xxx.fillStyle | #xxx.strokeStyle | #xxx.visible(true/false) | #xxx.opacity(0~1) | #xxx.fontSize | #xxx.instance ++/"],
              ["li",
